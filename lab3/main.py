...
rangeX = [(-15, 30), (5, 40), (5, 25)]
averXmax = (30 + 40 + 25)/ 3
averXmin = (5 - 15 + 5) / 3

maxY = 200 + int(averXmax)
minY = 200 + int(averXmin)
...
def coefficientOfRegression(x, y_aver, n):
    mx1 = sum(x[:, 1]) / n
    mx2 = sum(x[:, 2]) / n
    mx3 = sum(x[:, 3]) / n
    my = sum(y_aver) / n
...
def studentTest(x, y, y_aver, n, m):
    S_kv = dispersion(y, y_aver, n, m)
...
def fisherTest(y, y_aver, y_new, n, m, d):
...
def main(n, m):
    ...
    B = coefficientOfRegression(x, y_aver, n)

    Gp = kohrenTest(y, y_aver, n, m)
    print(f'Gp = {Gp}')
    if Gp < G_kr:
    ...
    ts = studentTest(x_norm[:, 1:], y, y_aver, n, m)
    print('\nКритерій Стьюдента:\n', ts)
