...
def planMatrix(n, m):
    print(f'\nМатрица планування для n = {n}, m = {m}')
...
def kriteriCochrana(y, y_aver, n, m):
    f1 = m - 1
    f2 = n
    q = 0.05
...
def kriteriyStudenta(x, y, y_aver, n, m):
    S_kv = s_kv(y, y_aver, n, m)
...
def kriteriyFishera(y, y_aver, y_new, n, m, d):
...
def check(X, Y, B, n, m):
    ...
    Gp = kriteriCochrana(Y, y_aver, n, m)
    print(f'Gp = {Gp}')
    if Gp < G_kr:
    ...
    ts = kriteriyStudenta(X[:, 1:], Y, y_aver, n, m)
    print('\nКритерій Стьюдента:\n', ts)
    ...
    if len(final_k) == 1:
        print('\nМодель не адекватна, кількість значимих коефіцієнтів = 1')
    else:
        print('\nМодель адекватна, кількість значимих коефіцієнтів більше 1')
    ...
    F_p = kriteriyFishera(Y, y_aver, y_new, n, m, d)
...
def main(n, m):
    X5, Y5, X5_norm = planMatrix(n, m)

    y5_aver = [round(sum(i) / len(i), 3) for i in Y5]
    B5 = findCoefs(X5, y5_aver)
